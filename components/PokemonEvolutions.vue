  ...
  <div>
    ...
    <div class="flex justify-center gap-2">
      <div v-if="evolution.pre" class="border border-amber-500 p-2 m-2">
        <h4 class="font-bold text-center border-b border-amber-500">Précédente</h4>
        <div class="mt-2">
          <NuxtLink v-for="evol of evolution.pre" :key="evol.pokedex_id" :to="`/pokemon/${evol.pokedex_id}`" class="flex flex-col items-center">
            <span>{{ evol.name }}</span>
          </NuxtLink>
        </div>
      </div>
      ...
      <div v-if="evolution.next" class="border border-amber-500 p-2 m-2">
        <h4 class="font-bold text-center border-b border-amber-500">Suivante</h4>
        <div class="mt-2">
          <NuxtLink v-for="evol of evolution.next" :key="evol.pokedex_id" :to="`/pokemon/${evol.pokedex_id}`" class="flex flex-col items-center">
            <span>{{ evol.name }}</span>
          </NuxtLink>
        </div>
      </div>
